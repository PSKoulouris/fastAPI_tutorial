...
#Function to creae a book minto the Book Table
def create_book(session: Session, book_in : BookCreate) -> Book: #use book and send it into...
    ...
    session.refresh(db_book)#refresh and empty variable to have it clean and ready for the enxt request in place
    return db_book

def get_all_books(session : Session, skip : int = 0, limit : int = 10) -> List[Book]:
    statement= select(Book).offset(skip).limit(limit) #use the functions to select from the database table
