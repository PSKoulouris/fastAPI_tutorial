...
async def get_all_books(session : AsyncSession, skip : int = 0, limit : int = 10) -> List[Book]:
    ...
    result = await session.exec(statement) #The initial value give is <<Promise>>
    return result.all()
...
"""
Synv Version: def delete_book()
            -> to call it: delete_book()

Async Version : async def delete_book()
            -> to call it: await delete_book()
